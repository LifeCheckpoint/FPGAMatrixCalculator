...
1.  **`ascii_num_sep_top` (输入缓冲)**:
    *   位于子系统顶层，负责接收 UART 数据并解析为整数。
    *   为 `matrix_op_selector` 提供解析后的指令数据。

2.  **`matrix_op_selector` (运算选择器)**:
    *   负责与用户进行交互 (通过 UART TX 发送提示)。
    *   从输入缓冲读取用户指令（如矩阵维度、ID、标量）。
    *   扫描 BRAM 以列出可用的矩阵。
    *   验证用户选择的合法性 (如维度匹配)。

3.  **`matrix_op_executor` (运算执行器)**:
